...
of the primary current, and both bridge polarities present the same network to
the tank, so there are three distinct linear circuits: IGBT conducting, diode
conducting, and the blocked bridge that carries no primary current at all.
"""
...
class Bridge:
    ...
    def state(self, gate, current):
        """Conduction state ``(index, polarity sigma, current sign)`` of the bridge.
    ...
    def drive(self, gate, current, v_bus):
        """Tank drive voltage, less the constant part of the conduction drop.

        The differential part n_dev r i is linear in the state and belongs in the
        primary loop resistance, so only sigma gain V_bus and the n_dev v0 offset
        appear here. The drop opposes the current, whatever conducts.
        """
        index, sigma, sign = self.state(gate, current)
        switch = self.conducting(index)
        drop = 0.0 if switch is None else self.devices * switch.v0 * sign
        return sigma * self.gain * v_bus - drop
